...
tagTIM_T tTimer2 = 
{
	...
	.tTimerHandle.Init.Period			= 1000-1,					/* 0.1S产生一次溢出 */
...
/* Timer3 配置 - 驱动 PB0, PB1 (电机1, 2) */
tagTIM_T tTimer3 = 
{
    .tTimerHandle.Instance = TIM3,
    .tTimerHandle.Init.Prescaler = 72-1,      // 72MHz / 72 = 1MHz (1us计数一次)
    .tTimerHandle.Init.Period = 20000-1,     // 1MHz / 20000 = 50Hz (20ms周期)
    .tTimerHandle.Init.CounterMode = TIM_COUNTERMODE_UP,
    .tTimerHandle.Init.ClockDivision = TIM_CLOCKDIVISION_DIV1,
    .tTimerHandle.Init.AutoReloadPreload = TIM_AUTOMATICOUTPUT_ENABLE,
    
    .ucPriority = 1,       // 建议加上优先级
    .ucSubPriority = 0,
};

/* Timer4 配置 - 驱动 PB6, PB8 (电机3, 4) */
tagTIM_T tTimer4 = 
{
    .tTimerHandle.Instance = TIM4,
    .tTimerHandle.Init.Prescaler = 72-1,
    .tTimerHandle.Init.Period = 20000-1,
    .tTimerHandle.Init.CounterMode = TIM_COUNTERMODE_UP,
    .tTimerHandle.Init.ClockDivision = TIM_CLOCKDIVISION_DIV1,
    .tTimerHandle.Init.AutoReloadPreload = TIM_AUTOMATICOUTPUT_ENABLE,
...
/* 定义电机 PWM 引脚的 GPIO 配置 */
// 注意：Mode 必须是 GPIO_MODE_AF_PP
tagGPIO_T motorGPIOGroup[] = 
{
    [0] = { .tGPIOPort = GPIOB, .tGPIOInit = { .Pin = GPIO_PIN_0, .Mode = GPIO_MODE_AF_PP, .Pull = GPIO_NOPULL, .Speed = GPIO_SPEED_FREQ_HIGH } },
    [1] = { .tGPIOPort = GPIOB, .tGPIOInit = { .Pin = GPIO_PIN_1, .Mode = GPIO_MODE_AF_PP, .Pull = GPIO_NOPULL, .Speed = GPIO_SPEED_FREQ_HIGH } },
    [2] = { .tGPIOPort = GPIOB, .tGPIOInit = { .Pin = GPIO_PIN_6, .Mode = GPIO_MODE_AF_PP, .Pull = GPIO_NOPULL, .Speed = GPIO_SPEED_FREQ_HIGH } },
    [3] = { .tGPIOPort = GPIOB, .tGPIOInit = { .Pin = GPIO_PIN_8, .Mode = GPIO_MODE_AF_PP, .Pull = GPIO_NOPULL, .Speed = GPIO_SPEED_FREQ_HIGH } },
};
